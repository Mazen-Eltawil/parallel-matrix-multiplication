...
import json
import logging
import logging.handlers  # Add explicit import for handlers
import os
import sys
import threading
...
# Add handlers after basicConfig
...
class MatrixMultiplication:
    ...
    def plot_performance(self, results: Dict) -> None:
        ...
        workers = results.get("workers", [1])  # Default to [1] if not present
        ...
        colors = plt.cm.Set3(np.linspace(0, 1, len(methods) * len(workers)))
        color_idx = 0
        ...
        for method_name, size_data in methods.items():
            ...
            for worker_count in workers:
                worker_key = "single" if worker_count == 1 else str(worker_count)
                times = []
                valid_sizes = []
                
                for size in sizes:
                    if size in size_data and isinstance(size_data[size], dict):
                        if worker_key in size_data[size]:
                            time_value = size_data[size][worker_key]
                ...
                if times:
                    label = f"{method_name}"
                    if worker_key != "single":
                        label += f" ({worker_key} workers)"
                    ax1.plot(valid_sizes, times, marker="o", 
        ...
        ax1.set_title("Matrix Multiplication Performance Comparison", fontsize=14, pad=20)
        ax1.grid(True, linestyle='--', alpha=0.7)
        ax1.legend(bbox_to_anchor=(1.05, 1), loc="upper left", fontsize=10)
        ax1.set_yscale("log")
        ax1.tick_params(axis='both', which='major', labelsize=10)
        
        # Reset color index for consistent colors between plots
        color_idx = 0
        ...
        if baseline_method in methods:
            ...
            for method_name, size_data in methods.items():
                if method_name == baseline_method or not isinstance(size_data, dict):
                    continue
                ...
                for worker_count in workers:
                    ...
                    for size in sizes:
                        try:
                            if (size in size_data and 
                                size in baseline_data and 
                                isinstance(size_data[size], dict) and 
                                isinstance(baseline_data[size], dict) and 
                                "single" in baseline_data[size] and 
                                worker_key in size_data[size]):
                                
                                baseline_time = baseline_data[size]["single"]
                                method_time = size_data[size][worker_key]
                                
                                if baseline_time > 0 and method_time > 0:
                                    speedup = baseline_time / method_time
                                    speedups.append(speedup)
                                    valid_sizes.append(size)
                        except Exception as e:
                            logging.warning(f"Error calculating speedup for {method_name}: {e}")
                    
                    if speedups:
                        label = f"{method_name}"
                        if worker_key != "single":
                            label += f" ({worker_key} workers)"
                        ax2.plot(valid_sizes, speedups, marker="o", 
                                color=colors[color_idx], label=label,
                                linewidth=2, markersize=8)
                        color_idx = (color_idx + 1) % len(colors)
        
        ax2.set_xlabel("Matrix Size (N×N)", fontsize=12)
        ax2.set_ylabel("Speedup (relative to NumPy)", fontsize=12)
        ax2.set_title("Speedup Comparison", fontsize=14, pad=20)
        ...
        # Save detailed results to JSON with proper formatting
...
def main():
    ...
    try:
        ...
        print("\nInitializing MatrixMultiplication...")
